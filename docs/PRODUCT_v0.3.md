...
| Material | **glassmorphism** ⟵ *corrected 2026-08-24* | **This row said `neo`, and its reason was false about the code that shipped.** It read: *"the only material with both a light and a dark scheme and no blur under it. Glass and liquid glass are `Modifier.blur` / `RenderEffect` — API 31+, with a fallback below that changes the look."* `#57` b never used either primitive: Compose has no backdrop filter at all, so glass and liquid glass are **translucent panels over a gradient backdrop** (`Modifier.gpPage`). Measured 2026-08-24: `Modifier.blur`, `RenderEffect` and `BlurMaskFilter` have **zero** call sites in `app/src/main`, and every `SDK_INT` gate in the app is in `notifications/` — so glass renders identically on `minSdk` 26 and on 35, and `AppMaterialTest` now fails the build if that stops being true. **What the old default cost:** `AppBackground.MATCH` (also the default) resolves neo to `PLAIN`, *"one flat tone, no lights at all"*, so a fresh install was opaque, unlit and flat, and all four presentation features `#57` shipped were off until the user found the picker. Ido reported the app as not matching the prototypes on 2026-08-21 **and again on 2026-08-24**, after all four had landed; he ratified this change the same day. `C6`'s `--edge` still applies and neo is still safe to default to — it is simply no longer the default. The widget-parity half of the old reason is **also** retired: `WidgetPalette` was neo-only on the same premise and is now a translucent panel too |
