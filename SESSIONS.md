...
|---|---|---|---|---|
| `50b-transaction-guard` *(round 5)* | **Closing this session's own last open issue rather than leaving it for a stranger.** `docs/PRODUCT_v0.3.md` §5.2 asserts *"two client transactions **already write** `goal.currentValue`"* with two line numbers; #49 removed both, and neither line holds a write. **Annotated, not rewritten** — §5.3's precedent, and for the same reason: the sentence is the *argument* that produced the decision (*"the pattern was never three sites converging on a server"*), so deleting it removes the reasoning. ⚠️ **One difference from §5.3, stated in the annotation:** that sentence becomes true when `C20` ships; **this one never does** — the work already happened, so it is dated as superseded rather than pending. **`docs/` is claimed by nobody** — verified against `c20-build-half`'s row, which owns `functions/**`, `firestore.rules`, four `data/firestore` files and `app/src/test/**`, and **no `docs/` path.** | `docs/PRODUCT_v0.3.md`, `CHANGELOG/2026-08-20/50b-transaction-guard.md` | none — no build, no device | 2026-08-20 |
| `c20-build-half` | **`C20`'s build half — the projection function, its two triggers, and `setDone` reduced to one write.** Brief: [`sessions/c20-build-half.md`](sessions/c20-build-half.md); design of record `docs/PRODUCT_v0.3.md` §5.2. Moves `points`/`level` and `challengeParticipant.score` off the client into one Firestore-triggered projection, deletes `publicProfiles.level` outright, adds the first field-level conditions to `firestore.rules`, and pins the Kotlin/TypeScript arithmetic with one shared fixture both test layers read. ⚠️ **`sessions/c20-build-half.md` is NOT in my paths** — `50b-transaction-guard` r3 holds `sessions/` and is **live** (last transcript turn 2026-08-19T23:34Z). Its `status: active` flip and its close to `sessions/done/` wait for that row to release. | `functions/**`, `firestore.rules`, `firestore-tests/**`, `firebase.json`, `app/src/main/java/com/idomarhaim/goalpilot/data/firestore/TaskRepositoryImpl.kt`, `app/src/main/java/com/idomarhaim/goalpilot/data/firestore/SocialRepositoryImpl.kt`, `app/src/main/java/com/idomarhaim/goalpilot/data/firestore/dto/Dtos.kt`, `app/src/main/java/com/idomarhaim/goalpilot/data/firestore/dto/Mappers.kt`, `app/src/test/java/com/idomarhaim/goalpilot/**` *(new fixture + tests)*, `CHANGELOG/2026-08-20/c20-build-half.md` | **`#gradle-daemon`**, **`#firebase-emulator`** — no device | 2026-08-20 |
