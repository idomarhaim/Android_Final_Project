...
>
> ⚠️ **`main` is still UNPUSHED and I did not push it.** Six commits, four of them mine and
> two foreign, and the foreign ones sit under **live rows** (`ai-goal-onboarding`,
> `manual-demo-script`) — precondition 5 stops there and says ask. Re-checked against
> `@{u}` at close-out: still unpublished. Ido's word is all that is missing.
