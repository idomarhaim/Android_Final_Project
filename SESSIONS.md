...
| `50b-transaction-guard` *(round 3)* | **`gh` is installed, the 🎬 gate was waived, and the eight open issues need a run order.** Correcting `CLAUDE.md`'s *"`gh` is NOT INSTALLED on this machine at all"*, which is now false — v2.97.0 portable at `%LOCALAPPDATA%\Programs\gh`, on the **User PATH**, **not authenticated** (`gh auth login` is Ido's). Then the wave-3 briefs the 🚥 line has been naming, against a dependency map verified at HEAD rather than read off the tickets. | `CLAUDE.md`, `docs/CLOUD-DEVICE.md`, `sessions/` *(new briefs)*, `CHANGELOG/2026-08-20/50b-transaction-guard.md` | none — no build, no device | 2026-08-20 |
