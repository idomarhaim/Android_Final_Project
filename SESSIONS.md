...
>
> ⚠️ **One item is waiting on that session.** `D1` resolved to *not a defect but an
> undecided model question* ("what should a challenge score from?"), which belongs
> in `TODO/TODO_FUTURE/ProductModel.TODO.future.md` beside `C7` — a file
> `product-model-map` owns. It was **not** moved; the verdict and its evidence sit
> under `D1` in `TODO/TODO_OPTIONAL/ProductReview.TODO.optional.md` for Ido to
> re-assign. If the map is still being charted, this is a candidate `C14` and the
> map is incomplete without it.
