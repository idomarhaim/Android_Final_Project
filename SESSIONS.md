...
|---|---|---|---|---|
| `fix-task-completion-feedback` | `/kickoff fix-task-completion-feedback` — close **[#3](https://github.com/idomarhaim/Android_Final_Project/issues/3)**: ticking a task is ~2 s of dead screen online and a silent no-op offline. **Ido's call 2026-08-08** — *keep* the `runTransaction` (it is what holds task/points/level/goal-progress together, and `FieldValue.increment` can express neither clamp nor a derived level), add an optimistic tick that is **undone with a message** when the write fails, then **measure the offline failure on-device** and add a connectivity pre-check only if that revert is slow enough to read as a lie. Sweeps the discarded-`Resource` shape, which the audit found at **five** sites, not the two the issue names | `app/src/main/java/com/idomarhaim/goalpilot/feature/goals/GoalDetailViewModel.kt`, `…/feature/goals/GoalDetailScreen.kt`, `…/feature/social/SocialViewModel.kt`, `…/data/firestore/TaskRepositoryImpl.kt`, `app/src/test/java/com/idomarhaim/goalpilot/feature/goals/GoalDetailViewModelTest.kt` *(new)*, `…/core/net/` *(new, only on escalation)*, `sessions/fix-task-completion-feedback.md`, `CHANGELOG/2026-08-08/fix-task-completion-feedback.md`, `kb-candidates/2026-08-08-fix-task-completion-feedback.md`, plus issue **#3** and one new issue for the Cloud-Function alternative | `#gradle-daemon`, `#emulator` **`Pixel_10_Pro_XL`**, and live **`goalpilot-56e30`** — re-measuring means really ticking a task, so it is restored and verified afterwards, as `product-device-pass` did | 2026-08-08 |
