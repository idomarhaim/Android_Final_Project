...
|---|---|---|---|---|
| `c11a-free-model-probe` | `/wayfinder 12 16` — resolve **[#16](https://github.com/idomarhaim/Android_Final_Project/issues/16)** (`C11a`), the map's AFK free-model probe: measure what `openai/gpt-oss-20b` on GROQ's free tier can actually do against a **fixed format**, in **Hebrew and English**, at the production temperature, using the three real prompt shapes already in `functions/src/index.ts`. Produces a reliability figure per shape per language, the named failure modes, and a verdict on **one wide call vs several narrow calls** — which prices `C1`, `C2` and `C11b`. **Reads `functions/src/index.ts`; does not edit it.** Writes only its own research asset, issue comments, board row, changelog and KB candidates | `docs/research/2026-08-08-free-model-format-probe.md` *(new)*, `CHANGELOG/2026-08-08/c11a-free-model-probe.md` *(new)*, `kb-candidates/2026-08-08-c11a-free-model-probe.md` *(new)*, `SESSIONS.md` *(this row, leased)*, plus issues **#16** and **#12** | **None of the board's.** No `#gradle-daemon`, neither AVD, no live `goalpilot-56e30` write — the probe calls **api.groq.com directly** with the key from git-ignored `functions/.env`, so it spends **GROQ free-tier quota** (30 RPM / 1 000 RPD) and nothing else. A concurrent session doing a live AI run would contend there and nowhere else | 2026-08-08 |
| `c15-language-switching` | `/wayfinder 12 15` — resolve **[#15](https://github.com/idomarhaim/Android_Final_Project/issues/15)** (`C15`), the map's HITL grilling ticket on the in-app language picker: which languages, where the chrome/content boundary falls, whether **AI-generated text** follows the picker (which lands straight on `C11a`/[#16](https://github.com/idomarhaim/Android_Final_Project/issues/16)), whether RTL rides the same switch, and per-device vs per-account persistence. **Planning only — no code.** Reads `app/src/main/res/`, `feature/profile/`, `data/prefs/` and `functions/src/index.ts`; **edits none of them** | `CHANGELOG/2026-08-08/c15-language-switching.md` *(new)*, `kb-candidates/2026-08-08-c15-language-switching.md` *(new, if any)*, `SESSIONS.md` *(this row, leased)*, plus issues **#15** and **#12** | **None.** No `#gradle-daemon`, neither AVD, no live `goalpilot-56e30`, no GROQ call — so it does not contend with `c11a-free-model-probe`'s quota nor with `fix-task-completion-feedback`'s device. The one singleton it does take is **Ido's attention**, which the board cannot enforce | 2026-08-08 |
