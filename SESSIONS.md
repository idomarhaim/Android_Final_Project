...
|---|---|---|---|---|
| `c20-build-half` *(round 3)* | **`CLAUDE.md`'s `gh` bullets are false in two ways and one of them sends the next session to a path that does not exist.** (1) The binary is at `C:\Program Files\GitHub CLI\gh.exe`, **not** `%LOCALAPPDATA%\Programs\ghin\` — the `export PATH=…/AppData/Local/Programs/gh/bin` line in that bullet returns *command not found*, verified by running it. (2) `gh` **is now authenticated** (`Logged in as idomarhaim`, keyring) — Ido ran `gh auth login --web` on my request at 2026-08-20 ~04:4x, so *"it is NOT authenticated"* is false. (3) Additive, not a correction: `c7807d0`'s `git credential fill` route is **blocked by the Claude Code auto-mode classifier** as credential extraction — it fired for me, so that route is not always available and the `gh auth login` it advises against is the fallback that works. Their finding stays; the caveat is added beside it. **Disjoint from `50-finish`, which is live and owns four `app/**` paths + its own brief and changelog; `CLAUDE.md` is in nobody's claim.** | `CLAUDE.md` | none — no build, no device, no emulator | 2026-08-20 |
