...
|---|---|---|---|---|
| `c20-build-half` *(round 2)* | **Ido: *"run the emulator, and you do the deploy"* — so both of round 1's open issues are being closed by this session rather than handed over.** (1) An **emulator round-trip test** for the two Firestore triggers, which were unit-tested but had never executed *as triggers*. (2) The **deploy** to `goalpilot-56e30` — `firebase login:list` reports the CLI already authenticated as Ido, and he has explicitly authorised the outward action. ⚠️ **And a defect this session introduced in round 1:** `TaskRepositoryImpl`'s new KDoc claims the owner's own point totals are *"summed from these facts on the device"*. **They are not** — `AuthRepositoryImpl.authState()` reads the stored `users/{uid}.points` and `UserDto.toDomain()` passes it straight through, so Dashboard, Profile and the widget all read the stored number. That also **resolves round 1's open design question**: the projection *must* write `users/{uid}.points`, and §5.2's *"no writer"* row describes an end-state nobody built. | `functions/**`, `app/src/main/java/com/idomarhaim/goalpilot/data/firestore/TaskRepositoryImpl.kt`, `CHANGELOG/2026-08-20/c20-build-half.md`, `docs/PRODUCT_v0.3.md` *(one annotation to §5.2's table row — released by `50b-transaction-guard` at `de696a6`)* | **`#firebase-emulator`**, **`#gradle-daemon`**, **the live project `goalpilot-56e30`** — no device | 2026-08-20 |
