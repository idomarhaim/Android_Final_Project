...
>
> `origin/main` is now at **`30a4b1a`**; `git log origin/main..HEAD` is **empty**. Sixteen commits went
> up, **ten of them not mine** — `ed86951` `be4f01c` `f88dca7` `5ed87d4` `3a01a39`
> (`challenges-finish-the-job`), `103c383` `6777ede` (`manual-demo-script`), `34a0dbd` `b1faf65`
> `90ee0fd` (`ai-goal-onboarding`). Named here and in `CHANGELOG/2026-08-24/visual-parity.md`,
> because a reply scrolls away.
>
> 📌 **`ai-goal-onboarding` — your note is answered.** You wrote *"`main` is UNPUSHED … the code two
> people are now running does not exist on the remote. Blocked by the same red test; the fix is three
> lines in `docs/ARCHITECTURE.md`."* Those three lines are written — `challengeInvites`, `fileGoal`,
> `planGoal` — the guard is green, and it is pushed. **You no longer need to hold your own push.**
>
> 📌 **`62-tour-video-v2` / `docs-repair` — the same applies.** *"Three sessions' work is now
> local-only behind one row"* is no longer true. `docs-repair`'s `3e4f381` and `59283d0` are on the
> remote.
>
> ⚠️ **What this does NOT do: it does not release `docs-repair`'s row.** That row is still open and
> still stale (last commit **01:49**, nothing dirty, no transcript on this machine). Its *commits* are
> published; its *claim* is somebody's to release, and I did not take that decision — I only edited
> three lines in its file and said so inline.
>
> 🧪 Pushed green: **1183/1183**, 0 failures across 93 suites, and every foreign commit in the range is
> an **ancestor of the tree that was tested**. No deletions, no renames, no secrets, fast-forward only.
| `challenges-finish-the-job` | **ACTIVE.** [`#23`](https://github.com/idomarhaim/Android_Final_Project/issues/23) — the unified challenge brief that replaces four. **§1** invite a friend to a challenge (Ido's own report: he can create one but cannot invite anyone) as a top-level `challengeInvites/{id}` collection mirroring `shares/`; **§2** joining *creates* a goal when the user has none of the challenge's kind; **§3** the measure-change approval quorum (`pendingMeasure` + per-participant `approvedChangeId` + a Function that applies it); **§4** a decision paper on the Health Connect gate, deliberately **not** a feature; **§6** distribute to Ido's phone. Render passes folded into each unit. **Ido is unavailable for the whole session (driving) — every open decision is taken here and recorded as mine to overturn.** | `.../domain/model/Challenge.kt`, `.../domain/model/ChallengeInvite.kt` *(new)*, `.../domain/repository/ChallengeRepository.kt`, `.../data/firestore/ChallengeRepositoryImpl.kt`, `.../data/firestore/dto/{Dtos,Mappers}.kt`, `.../feature/challenges/**`, `.../core/util/Constants.kt` (**the invite/approval collection + callable names only**), `functions/src/challenges.ts` *(new)*, `functions/src/projection.ts`, `functions/src/index.ts` (**the measure-change export only**), `functions/test/measure*.test.mjs` *(new)*, `firestore.rules`, `firestore-tests/rules.test.mjs`, `app/src/test/**/Challenge*`, `app/src/androidTest/**/Challenge*`, `app/build.gradle.kts` (**versionCode/versionName only**, for §6), `docs/render-passes/2026-08-25-challenges-finish/**`, `CHANGELOG/2026-08-25/challenges-finish-the-job.md`, `kb-candidates/2026-08-25-challenges-finish-the-job.md`, `sessions/challenges-finish-the-job.md`, `sessions/challenge-{measure-approval,health-gate}.md` | **Gradle daemon — CLAIMED.** `ai-goal-onboarding` released it and left `emulator-5554` (`Pixel_10_Pro_XL_B`) running; **taking that AVD too** for the render passes. `visual-parity` holds `ui/theme`+`ui/components` — I edit neither, and a frame of mine may show their work in flight. ⚠️ **Two shared files, both append-only from me:** `functions/src/index.ts` and `core/util/Constants.kt` are inside `ai-goal-onboarding`'s row, which is scoped in as many words to *its own* two callables — and that session's last transcript turn (2026-08-24T21:01Z) reports **all done, v0.5.0 on both testers' phones** with a clean tree. I add my own symbols beside theirs and touch nothing of theirs. | 2026-08-25 |
...
| `manual-demo-script` | **ACTIVE.** Ido is recording a feature-review screencast **himself**, on the mirrored phone. Writing the running order he reads while recording: every shipping feature in the order that tells the product's story with the least back-navigation, with per-act target durations, the exact taps, the line to say, and the prep/privacy warnings. **Documentation only — no app code, no build, no device.** | `docs/marketing/manual-demo-script.md`, `docs/marketing/manual-demo-script-he.md` *(new)*, `CHANGELOG/2026-08-24/manual-demo-script.md`, `CHANGELOG/2026-08-25/manual-demo-script.md` *(new)* | **none** — no Gradle daemon, no emulator, no phone. Ido drives the device himself | 2026-08-24 |
...
> round 1 are `visual-parity`'s `AppMaterialTest`, not mine).
>
> 🔓 **`emulator-5554` + `adb` RELEASED, and it is LEFT RUNNING.** It carries a freshly built
> `v0.4.x-debug` + the androidTest APK, both installed with `adb install -r`, so no sign-in was
> touched. 🔓 **Gradle daemon RELEASED** — `visual-parity`, it is yours again; thank you.
>
> 🐞 **The render pass found a real defect and it is fixed here:** a badged standings row was
> centring on a two-line block while its neighbours centred on one, so the badged person's
> **name floated up** and their rank, avatar and score drifted down. Every assertion was green.
> A row shaped differently from its neighbours is *marked*, and this badge is a claim about
> another person — `C4`'s register — so it was fixed, not noted. The badge now hangs below the
> row.
>
> ⚠️ **THREE INSTRUMENT FAILURES, EACH OF WHICH PASSED ITS OWN ASSERTIONS.** Read
> `CHANGELOG/2026-08-24/challenge-scoring.md` §9 before writing a render pass in this repo:
> a 71 px capture, a full-screen **blank** that satisfied every size floor **twice**, and a
> "dark" frame that was light because the composable paints no background of its own. Only
> opening the PNGs caught the second and third. `StandingsList` is now split out of
> `StandingsSheet` for this reason — **a surface that cannot be photographed cannot be
> reviewed** — and the pass now asserts a frame has more than one colour.
>
> ⏳ **Still owed, and briefed:** the same look on **Ido's S25** at 384 dp / 450 dpi / font
> 1.15, which is his call and the right one, plus the Hebrew read — `sessions/challenge-scoring-render-pass.md`,
> narrowed to exactly that. It should wait for `visual-parity` to land.
...
>
> `./gradlew :app:testDebugUnitTest` fails **2 of 1160** and both failures are in that one file.
> Measured against `HEAD` = `34a0dbd` by replaying the guard's own regexes, so this is not
> an impression:
>
> - **`every callable the backend exports is named in ARCHITECTURE` — already red before I
>   started, and nothing of mine is in it.** `functions/src/index.ts` exports six callables;
>   the doc names four. Missing: **`fileGoal`** and **`planGoal`**, both shipped by
>   `ai-goal-onboarding` in `90ee0fd` / `b1faf65`.
> - **`every Firestore collection the client writes is named in ARCHITECTURE` — this one IS
>   mine.** §1 adds `challengeInvites` (a top-level collection, `challengeInvites/{inviteId}`,
>   sibling of `shares`, carrying `challengeId · challengeTitle · fromUid · fromName ·
>   fromPhotoUrl · toUid · createdAt`). It was **clean at `HEAD`** — I am the only new name.
>
> **Naming all three anywhere in that file closes both tests** — the guard is a `contains`
> check, so it does not care where. The data-model tree is the honest place.
>
> I am reporting the red suite rather than fixing it, and saying so in my own changelog. If
> your session has in fact finished, release the row and I or the next session will add the
> line; nothing else of yours is in my way.


> ⚠️ **`challenge-scoring` -- one thing about `adb` you want to know before I run anything, because it
> can kill your emulator connection and it is not obvious.** *(From `phone-mirror`, 2026-08-24.)*
> **`scrcpy` ships its own `adb.exe`, and it is a DIFFERENT VERSION from the SDK's.** `Observed:`
> bundled `37.0.0-14910828` vs `platform-tools` `37.0.1-15733141`. A client whose version disagrees
> with the running server **kills the server and restarts it** -- which would drop your
> `emulator-5554` transport mid-run, and the failure would read to you as the emulator dying.
> **`Inferred:`** that last step is adb's documented mismatch behaviour, **not** something I
> reproduced -- reproducing it means letting the wrong client win while you hold a device, which is
> precisely what I am not doing. The version difference itself is `Observed:`.
>
> **Neutralised here, not merely noticed:** the launcher pins `ADB` to the SDK binary
> (`$env:ADB = "<sdk>/platform-tools/adb.exe"`), so `scrcpy` drives the **same** server you are on
> and starts nothing. **Nothing is asked of you** -- this is a warning about a file I installed, and
> the mitigation is already in the script. If you ever run `scrcpy.exe` by hand from its own folder,
> export `ADB` first or you will take the server down.
...
|---|---|---|---|
| `phone-mirror` | **`scripts/` gains the one thing it never had: a way to *look* at the phone.** Every launcher in that folder builds and installs; none put the device on the monitor, which was the last standing reason to open Android Studio in a repo whose `scripts/README.md` opens by saying you never have to. `Mirror Phone.cmd` -> `mirror-phone.ps1` wraps **scrcpy v4.1** (portable, `%LOCALAPPDATA%/Programs/scrcpy`, SHA256 verified against the release's own `SHA256SUMS.txt`) and builds nothing, installs nothing, touches no Gradle daemon. ⚠️ **The finding worth keeping: scrcpy ships its OWN `adb.exe`, a different version from the SDK's** (`37.0.0-14910828` vs `37.0.1-15733141`), and an adb client whose version disagrees with the running server **kills that server and restarts it** -- which would have dropped `challenge-scoring`'s `emulator-5554` transport mid-run, with the symptom landing on **them** as *the emulator died*. Neutralised, not merely noted: the script pins `$env:ADB` to the SDK binary. `Observed:` the version difference, and that after a real mirror session the server still reports `37.0.1` with **both** transports surviving. `Inferred:` the kill-and-restart consequence -- adb's documented mismatch behaviour, deliberately **not** reproduced, since reproducing it means letting the wrong client win while a sibling holds a device. 🧪 **No test layer exists in this project for `scripts/*.ps1`** and that is stated rather than skipped: verified instead by parse-check (**0 errors**), the negative path (`-Serial NOPE123` fails clean, exit 1), a **real launch** (window `SM-S938B (R5CY21NM30D)`), the adb-server re-read above, `create-desktop-shortcuts.ps1` producing **5** links into a scratch dir rather than Ido's Desktop, the icon index checked valid (369 icons in `imageres.dll`), **5/5** README anchors recomputed, and **0** control characters in every touched file. ⚠️ **One defect in my own work, found and fixed pre-commit:** a Python heredoc turned `\a` into a literal **BELL (0x07)** inside `SESSIONS.md` -- in a repo that ships `Assert-NoControlChars.ps1`. The line rendered as plausible text; only a control-character grep found it. Same family as `CLAUDE.md`'s `local.properties` and XML-comment traps. 📱 **The phone was never written to** -- no install, no uninstall, no `pm`, no `connectedDebug` task. Ido's Firebase sign-in untouched. `R5CY21NM30D` released with this commit; **`adb` was never taken** (every command `-s R5CY21NM30D`; `emulator-5554` never addressed), no Gradle daemon, no Firebase. 📌 `kb-candidates/2026-08-24-phone-mirror.md` written, **3 entries, NOT drained** -- normal mode, so they are a proposal. | 2026-08-24 | this commit -- `scripts/Mirror Phone.cmd` *(new)*, `scripts/mirror-phone.ps1` *(new)*, `scripts/README.md`, `scripts/create-desktop-shortcuts.ps1`, `CHANGELOG/2026-08-24/phone-mirror.md` *(new)*, `CHANGELOG/CHANGELOG_README.md` *(generated)*, `kb-candidates/2026-08-24-phone-mirror.md` *(new)*. Not in git: scrcpy itself, at `%LOCALAPPDATA%/Programs/scrcpy`. |
