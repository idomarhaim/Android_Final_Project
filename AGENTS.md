...
- **Subagent gate:** before spawning ANY subagent — explain why, which model, why that model, and get user approval (no read-only exemption). A harness fan-out default (fleet/workflow presets) does not lift the gate.
