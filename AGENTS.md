...
- **[`C:\Dev\JARVIS\kb\index.md`](file:///C:/Dev/JARVIS/kb/index.md) — the cross-project knowledge base, and it is mostly about *this repo*.** **65 of its 76 `kb/dev/` pages name GoalPilot.** One row per page with a summary, so it is a *lookup* surface, not a reading list.
  - **Read it at three moments, because nothing else will make you:** before your **first device, emulator, deploy or Firebase command**; when something **surprises you** (a command fails in a way you did not predict); and before you spend more than a few minutes **working around** anything. Every one of those is a moment somebody has already been.
  - **Why this line exists at all.** The methodology has a fully triggered path *into* the KB — `/kb-flag` → `kb-candidates/` → `/kb-ingest`, firing at the commit trigger, on a session split, on `/handoff`, on `/kickoff` — and, until 2026-08-20, **no trigger anywhere that made anyone read one back**. `#9` re-derived `android-device-verification.md` §8 from scratch and lost a render capture doing it, having searched the KB twice that session — both times for a *destination to write to*, never for an *answer to read*.
  - The pages this repo trips over most: [`android-device-verification.md`](file:///C:/Dev/JARVIS/kb/dev/android-device-verification.md) (emulator, instrumented runs, and the fact that `connectedDebugAndroidTest` **uninstalls the app**) · [`firestore-write-semantics.md`](file:///C:/Dev/JARVIS/kb/dev/firestore-write-semantics.md) (cache vs server, offline writes) · [`powershell-encoding-traps.md`](file:///C:/Dev/JARVIS/kb/dev/powershell-encoding-traps.md) · [`look-at-your-own-output.md`](file:///C:/Dev/JARVIS/kb/dev/look-at-your-own-output.md) (how verification fails silently) · [`claude-code-surfaces.md`](file:///C:/Dev/JARVIS/kb/dev/claude-code-surfaces.md) (session identity, transcripts) · [`tool-adoptions.md`](file:///C:/Dev/JARVIS/kb/dev/tool-adoptions.md) (which tools are adopted here, and on what terms).
...
- **Concurrent sessions — claim by path, or don't run them at once.** Running two sessions in one working tree is opt-in, not the default, and legitimate only when the working sets are **disjoint**. The claim board is [`SESSIONS.md`](SESSIONS.md) at the repo root: **its existence is the trigger** — if the file is there, read it before your first edit, claim before your first write, release when done (a stale claim blocks work nobody is doing). Do not judge for yourself whether a sibling is live; that fact is *in* the board, so skipping it leaves you with no evidence you were allowed to. Claim **paths**, not themes — `ui/theme/`, not "the UI" — because only paths are checkable, and confirm your own row rather than inheriting one another session wrote for you from observed changes (that is a report, and it understates your footprint). **The board belongs to the repo being edited, not to the directory you started in** — a session that starts in A and writes into B and C owes a row on all three, read before its first write there; cross-repo work is routine rather than exotic (a `/kb-ingest` into the central bundle *is* cross-repo — candidate in the project, page in the bundle — as are template sweeps and bundle drains), and claiming only where you started is not a lesser claim but no claim, one repo over. **One exception, the ceremony rule's:** a *mechanical sweep* — a verbatim projection refreshed by a script, one commit into a clean tree — records itself in that board's *Recently released* rather than claiming, because a row created and cleared inside a single commit protects nothing; **the boundary is judgement, not repo count**, so the moment the visit involves a decision (prose, a conflict, a file a sibling could hold) it owes a row like any other, and unsure means you owe one. `Assert-SessionBoard.ps1 -Surface` puts every roster board carrying live claims in context at session start, so this is information you hold before your first edit anywhere. Shared singletons (build daemon, the git index, device/emulator, live cloud env) need a named owner and are exclusive — check them before your first **build or device command**, not only before your first edit. **In a board repo, blanket staging is banned outright** — `git add -A`, `git add .`, `git commit -a` — not merely "while a sibling is live"; a machine-wide `PreToolUse` hook (`C:\Dev\JARVIS\scripts\Assert-SessionBoard.ps1`) turns one into a permission prompt. The agent recommends, the user assigns: on seeing unclaimed work that fits, append one line — `🧭 **Claim:** <candidate> → owns <paths>; conflicts: <none|paths>` — and wait; never self-assign. Entry point `/claim`. Full rule: `C:\Dev\JARVIS\rules\agent-topology-and-model-routing.md` §5.
