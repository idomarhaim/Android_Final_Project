...
- **The candidate list is a file, not a memory — `kb-candidates/YYYY-MM-DD-<session-label>.md`.** Created at this repo's root on the session's first flag, appended to by every later one, riding the next commit of any kind. **Written in every mode:** `AUTO MODE` gates whether the list *drains* without asking, never whether it *exists*. **Each entry stands alone** — Claim (assertive, as it will read on the page) · Why (the reasoning and the rejected alternatives; this is the part a transcript would have held, so this is the part that must be here) · Destination · Anchors (committed paths, `none` where there honestly are none) · Supersedes · Status. An agent draining a file **must not read the originating session's chat history**: it is machine-local, outside git, invisible to the other agent, and truncated by compaction, so it is absent exactly when it would be reached for. An entry too thin to write a page from **stops and asks the user** — it is never reconstructed from a log. **Session-start duty:** in a repo that has `kb-candidates/`, list it before the first unit of work and report a non-empty result — the file existing is the trigger, the same as `SESSIONS.md`. That sweep, not the commit trigger, is what stops a forgotten drain from being lost: sessions end without anyone declaring them over. On a drain, `/kb-ingest` deletes a fully-drained file and **rewrites a partly-drained one down to the survivors** (original numbering kept, reasons written into `Status`, always-ask entries moved to a `## Standing — always-ask` section); the journal entry names the source candidate file **and its repo**, which is the candidate→page tie whenever the bundle lives in a different repo. Full rule: `C:\Dev\JARVIS\rules\memory-promotion.md`.
